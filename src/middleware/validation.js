const validateWorkout = (req, res, next) => {
    ...
    if (typeof exercise !== 'string') {
        ...
        // })
        return next(new Error("Exercise must be a string"))
    }
    if (typeof sets !== 'number' || typeof reps !== 'number') {
        // return res.status(400).send({
        //     error: "Sets and reps must be number"
        // })
        return next(new Error("Sets and reps must be number"))
    }
    if (sets <= 0 || reps <= 0) {
        // return res.status(400).send({
        //     error: "Sets and reps must be greater than 0"
        // })
        return next(new Error("Sets and reps must be greater than 0"))
    }
    next()
}
...
const validateSignup = (req, res, next) => {
    ...
    if (password.length < 6) {
        ...
        // })
        return next(new Error("Password should be 6 characters atleast."))
    }
...
const validateUpdateProfile = (req, res, next) => {
    ...
    if (req.body.password && req.body.password.length < 6) {
        ...
        // })
        return next(new Error("Password should be at least 6 characters."))
    }
    next()
}
